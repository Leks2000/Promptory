-- =============================================
-- Promptory - Setup Admin & Test Data
...
-- ==================== 1a. GET YOUR USER ID ====================
-- Сохраним ID в переменную для удобства
DO $$
DECLARE
  my_user_id UUID;
BEGIN
  -- Получаем ID (LIMIT 1 на случай дубликатов)
...
-- Добавляем тестовые промпты в библиотеку
-- ВАЖНО: Используем CTE для получения ID один раз
...
-- ==================== 4. ADD SOME TEST REPORTS (for testing moderation) ====================
-- Добавляем тестовые репорты для проверки модерации
...
VALUES (
  gen_random_uuid(),
  (SELECT id FROM auth.users WHERE email = 'alexhalle393@gmail.com'),
  (SELECT id FROM public.library_prompts WHERE title = 'Professional Code Review'),
  'Low Quality',
...
VALUES (
  gen_random_uuid(),
  (SELECT id FROM auth.users WHERE email = 'alexhalle393@gmail.com'),
  (SELECT id FROM public.library_prompts WHERE title = 'Professional Email Writer'),
  'Spam',
